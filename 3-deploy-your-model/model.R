...
# let's consider that we have high standards and only pass if grade higher, or eq, than 11
grades_raw$final <- factor(ifelse(grades_raw$G3 >= 11, 0, 1), 
                           labels = c("pass", "fail"))
...
set.seed(2020)
grades_raw$G2 <- ceiling(grades_raw$G2*runif(n=length(grades_raw$G2), min=0.5, max=.9999999999))
